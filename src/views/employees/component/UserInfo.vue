      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <img-upload ref="staffPhoto"></img-upload>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <img-upload ref="myStaffPhoto"></img-upload>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      // 如果获取到头像,没上传图片staff为空格字符串,空格字符串为true能找到就会保存到头像,去掉空格后找不到就不会赋值给fileList显示图片
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // 将头像赋值给上传组件数组对象形式
        this.$refs.staffPhoto.fileList = [
          { url: this.userInfo.staffPhoto, upload: true }, //upload为true表示上传成功
        ];
      }
    },
    // 获取个人详情信息
    ...
    async saveUser() {
      ...
      }); //保存头像为当前上传的图片,如果fileList没有图片就存有空格字符串,接口才能保存
